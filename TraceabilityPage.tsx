...
export default function TraceabilityPage() {
  ...
  // B12: accurate mapping of 6 display steps to 4 processing statuses
  const STEP_COMPLETED_STATUSES: string[][] = [
    ['raw', 'scoured', 'combed', 'ready'], // 0: shearing — completed once batch exists
    ['scoured', 'combed', 'ready'],         // 1: scouring
    ['combed', 'ready'],                    // 2: combing
    ['ready'],                              // 3: felting
    ['ready'],                              // 4: sterilizing
    ['ready'],                              // 5: finished
  ];
          ...
          <div className="relative">
            ...
            <div className="space-y-6">
              {PROCESS_STEPS.map((ps, idx) => {
                  const completed = STEP_COMPLETED_STATUSES[idx]?.includes(batch.processing_status || 'raw') ?? false;
                const Icon = ps.icon;
                return (
                  <motion.div
                    key={ps.labelEn}
