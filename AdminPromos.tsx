...
const AdminPromos = () => {
  ...
  const [form, setForm] = useState({
    code: '', discount_type: 'percent', discount_value: '', min_order_nzd: '0',
            ...
            <TableBody>
              {promos.map(p => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>{p.discount_type === 'percent' ? '百分比' : '固定金额'}</TableCell>
                  <TableCell>{p.discount_type === 'percent' ? `${p.discount_value}%` : `NZ$${p.discount_value}`}</TableCell>
