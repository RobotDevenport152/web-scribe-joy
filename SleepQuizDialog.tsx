...
export function SleepQuizDialog({ open, onOpenChange }: Props) {
  ...
  const getRecommendation = () => {
    const budget = answers[2];  // budget / mid / premium
    const season = answers[3];  // spring_autumn / winter / all

    if (budget === 'premium') {
      const slug = season === 'winter' ? 'premium-luxury-winter' : 'premium-luxury-spring-autumn';
      return { zh: '高奢款羊驼被', en: 'Premium Luxury Duvet', slug };
    }
    if (budget === 'mid') {
      const slug = season === 'winter' ? 'luxury-winter'
        : season === 'spring_autumn' ? 'luxury-spring-autumn'
        : 'luxury-summer';
      return { zh: '轻奢款羊驼被', en: 'Luxury Alpaca Duvet', slug };
    }
    const slug = season === 'winter' ? 'classic-winter' : 'classic-spring-autumn';
    return { zh: '经典款羊驼被', en: 'Classic Alpaca Duvet', slug };
  };
